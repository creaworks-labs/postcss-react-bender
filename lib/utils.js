...
export function splitComponentSelector(key) {
  const REGEX_IS_COMPONENT_STYLE = /((^[A-Z][^\s.:]*)|^\*)((\.|:{1,2})[a-z-A-Z_0-9]+)*$/;

  const component = key.replace(REGEX_IS_COMPONENT_STYLE, "$1");
  const selector =
    key.length > component.length ? key.substr(component.length) : undefined;
