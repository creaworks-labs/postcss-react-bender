...
function isPseudoStyle(selector) {
  if (!REGEX_IS_PSEUDO_STYLE.test(selector))
    return false;

    // console.log('aaa',key, tokenType)
  if (supportedPseudo.indexOf(selector) == -1)
    return false

  // console.log('pseudo', selector, supportedPseudo.indexOf(selector))  
...
export function splitStyleKey(styleKey) {
  ...
  const selector =
    styleKey.length > component.length ? styleKey.substr(component.length) : undefined;
